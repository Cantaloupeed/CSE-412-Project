...
Fetches menu items for a dining hall and meal period.

Query parameters:

- `hall`: dining hall name
- `meal`: meal type such as `Breakfast`, `Lunch`, or `Dinner`

Example:

```text
GET /menu?hall=Barrett&meal=Breakfast
```
...
[
  {
    "id": 1,
    "name": "Turkey_Sausage_Taco",
    "ingredients": "Potatoes, Corn_Tortilla, Turkey_Sausage",
    "calories": 210,
    "protein": 6
  }
]
...
Adds a new food item to a selected dining hall and meal period.
...
Deletes a food item and its related ingredients.
...
- Add campus-level filtering across Tempe, Downtown Phoenix, Polytechnic, and West Valley campuses.
- Add cost comparison tools using `Menu_EntranceCost`.
- Add dietary preference filters such as vegetarian, vegan, halal, allergen-aware, or high-protein options.
- Implement full update functionality for existing menu items.
- Add search by food item or ingredient.
- Add environment variable support for database credentials.
- Improve data import and refresh workflows for official ASU Dining information.
- Add user-facing sorting by calories, protein, dining hall, meal period, and price.
